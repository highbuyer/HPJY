...
class 智能体:
    def __init__(self, 动作数, 输入维度, 优势估计参数G=0.9999, 学习率=0.0003, 泛化优势估计参数L=0.985,
        ...
        model = get_model(config, 30, 模型名称)
...
#         self.文件名集 = []
    ...
    #     操作序列 = torch.from_numpy(状态['操作序列'].astype(np.int64)).to(device)
    #     # 图片张量=torch.from_numpy(状态['图片张量']).cuda(device)
    #     图片张量 = torch.from_numpy(状态['图片张量']).to(device)
    ...
    def 选择动作(self, 状态, device, 传入动作, 手动=False):
        ...
        # 避免计算不必要的梯度
        with torch.no_grad():
            # 将numpy数组转换为张量，并移动到指定的设备上，统一数据类型为torch.float32
            操作序列 = torch.from_numpy(状态['操作序列'].astype(np.int64)).to(device)
            图片张量 = torch.from_numpy(状态['图片张量']).to(device).float()
            trg_mask = 状态['trg_mask']
